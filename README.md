...
- Python, JavaScript, C 言語を使って公共データを処理し、それを仙台の高齢化地域をブラウザ上で閲覧できるようにします。
- 私が住んでいる仙台の GIS（地理情報システム）データに興味があったのが動機です。
- コロナウイルスでデータ可視化に注目が集まる今日この頃、Python や JavaScript でのデータ処理に慣れたかったのです。
...
5. `pipenv install`
6. `mkdir csv`
7. `python3 analyze.py`
   - This generates a CSV
   - Configure bool flags to generate optional CSV / JSON / histogram figure
...
- よく見ると、「穴が空いた形状の町」では余計な線が描画されてしまっている。Matplotlib で`Polygon`ではなく`Path`を使うと解決するかも？
...
2. `clang -o parse parse.c` with Clang, for example
3. `./parse` on Linux, for example
...
- OpenStreetMap に高齢化情報をプロットするファイル。
- `vis-map.js`内部で以下の Python で出力されたデータを使っている。
- 老年人口比率（0%-100%）を HSL の色相（240 度-0 度）に置換して表示色を決定する。
...
- データの見せ方は難しい。今後さらに要検討。

![HSV Variant](./img/hsv_0_240.png)

### `age_structure.py`

仙台市の年齢別・町名別・性別人口データのエクセルファイルを整形して CSV に出力するファイル。

1. 元データのエクセルファイルは区と性別ごとに複数のスプレッドシートにわかれてしまっているので集約する。
1. 「大字」と「小字」で人口がダブルカウントされているため、大字に集約する。例えば「種次字中屋敷」「種次字番古」「種次字南番古」「種次（大字計）」という行が連続しているので「種次」という大字名を記憶し、それを手がかりにして「種次字」で始まる地名の行を削除する。
1. 列の見出しが日本語表記のままではデータ処理で扱いにくいので、英字の略号に変換する。
1. ここまでの結果（５歳刻みでの町別・年齢別人口構成データ）を CSV に出力する。

### `position.py`

町ごとの年齢人口データと、町ごとの位置情報をJOINするファイル。

1. `age_structure.py`の出力結果である年齢別人口構成データを DataFrame A として取り込む。
1. 仙台市の各町・丁・大字の代表点の GPS 座標データが入ったエクセルファイルを処理する。 データを整形して DataFrame B とする。
1. 「丁目」の書式が年齢構成ファイルでは数字（４丁目）だが、位置情報ファイルでは漢数字（四丁目）になってしまっているので統一する。
1. DataFrame A と DataFrame B を、町名と区名をキーにして JOIN する（町名だけをキーにすると失敗する。例えば青葉区と太白区両方に「茂庭」という町名があるため）。
1. ここまでの集計結果（町別の人口三区分の人口構成と町の位置情報）を返却する。

### `analyze.py`

町ごとの年齢人口データを集計して、各種の指標を算出し、その結果を保存するプログラム。

1. `position.py`を内部で呼び出す。
1. これを元に人口三区分（年少人口、生産年齢人口、老年人口）毎の人数・人口比を集計し、また高齢化率、従属人口比率、老年化指数、男女比も算出する。
1. この集計結果を CSV または JSON に保存し、またヒストグラムにする。
...
- 仙台市の小字の位置情報ファイルを処理するファイル。
- 元データが 16 万行もあるため、処理速度のためこのファイルのみ C 言語を選んだ。
